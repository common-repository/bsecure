...
class Bsecure_Checkout extends WC_Bsecure {	
	...
	*/
	...
	const BSECURE_LIVE_WEBHOOK_URL = 'https://cron.bsecure.pk/v1';
	const BSECURE_LIVE_BASE_URL = 'https://api.bsecure.app/v1';
	...
	public function __construct(){
		...
		add_action( 'plugins_loaded', array($this, 'load_bsecure_gateway_class' ),11 );		
		...
		add_action('woocommerce_checkout_process', array($this, 'validate_custom_checkout_fields'),20);
		...
		add_action( 'woocommerce_order_status_changed', array($this,'bsecure_action_woocommerce_order_status_changed'), 10, 2 );
			...
			if(!empty($order_data->order_ref)){
				...
				if(!empty($wc_order)){			
					...
					if($order->get_status() == Bsecure_Checkout::STATUS_FAILED && $placement_status == Bsecure_Checkout::BSECURE_PROCESSING_STATUS){
						...
						$isStatusUpdated = $this->updateOrderStatus($order,$order_data);
						if($isStatusUpdated)
						$this->bsecure_action_woocommerce_order_status_changed($wc_order->ID, true);

						$return_json = ['status' => true, 'msg' => __("Order status updated successfully.", 'wc-bsecure'),'bsecure_order_id' => $wc_order->ID];
					}else{

						$order->update_status($this->woocommerceStatus($placement_status,$order_data),"",true);
						...
						$this->bsecure_action_woocommerce_order_status_changed($order->get_id(), true);					
						...
						if($order_id > 0){
							...
							if($order->get_status() == Bsecure_Checkout::STATUS_CANCELED || $order->get_status() == Bsecure_Checkout::STATUS_FAILED){
								...
								if($order->get_payment_method() == 'bsecures' || ($order_data->payment_method->id == 1 && $order->get_payment_method() == 'cod' )){
									...
									$msg = __("This order is no more related to bSecure.", "wc-bsecure");
								}
	...
	*/
	public function manage_wc_order(){		
		...
		if(!empty($wc_order)){	
			...
			if($order->get_status() == Bsecure_Checkout::STATUS_CANCELED ){				
			...
			} else if ($order->get_status() != Bsecure_Checkout::STATUS_FAILED) {
		...
		if($order_id > 0){
			...
			if(($order->get_status() == Bsecure_Checkout::STATUS_CANCELED || $order->get_status() == Bsecure_Checkout::STATUS_FAILED) && $order_data->placement_status == Bsecure_Checkout::BSECURE_FAILED_STATUS){				
			...
			$this->updateOrderStatus($order,$order_data);
			exit;
	...
	public function createOrderFromBsecureToWc($order_data, $reponse_type = 'json') {
		...
		if(!empty($order_data->order_type) && !empty($order)){
			...
			if(strtolower($order_data->order_type) == Bsecure_Checkout::ORDER_TYPE_PAYMENT_GATEWAY){
				...
				if($order->get_payment_method() == 'bsecures' || ($order_data->payment_method->id == 1 && $order->get_payment_method() == 'cod' )){
					...
					return $this->updateOrderPaymentGateway($order, $order_data, $isCustomer, $reponse_type);
				}else{
	  ...
	  // Get the customer country code
	    ...
	    if(!empty($order_data->shipment_method->id)){
	    	...
	    	//Remove Shipping if exists when updating order
	    	
			$shipping_items = ($order->get_items( 'shipping' )); 
			if(!empty($shipping_items)){
				foreach ($shipping_items as $key => $value) {
					if ($value->get_name() == $order_data->shipment_method->name ) {	
						if ( method_exists( $value, 'get_id' ) ) {					
							$order->remove_item($value->get_id());
							$order->calculate_totals();	
						}
					}
				}
	...
	private function createOrderFromCartToBsecure($accessToken){
		...
		if ( ! WC()->cart->is_empty() ) {
       	...
       	$sale_price = !empty($getProductDetail->get_sale_price()) ? $getProductDetail->get_sale_price() : $regular_price;
			...
			$request_data['currency_code'] = !empty($order->get_currency()) ? $order->get_currency() : get_option('woocommerce_currency');
		...
		// since 1.7.4 update all btn styling from bSecure server 
		$this->updateBtnUrlsfromBsecure($response);
   ...
   */
	public function ajax_order_to_bsecure(){
		...
		// Create Order //
		$this->access_token = __("No token needed","wc-bsecure");
	...
	*/
	public function woocommerceStatus($placement_status, $order_data = []){
		...
		switch ($placement_status) {
			...
			case 3:
				...
				$order_status = $this->getConfimredStatus($order_data);
   ...
   */
	...
	*/
	 ...
	 */
	public function updateOrderPaymentGateway($order, $order_data, $customer, $reponse_type = 'json'){		
	...
	public function custom_product_api_response($response, $post, $request) {
		$params = $request->get_params();
		// Check if request is for bsecure portal or not
		if (is_wp_error($response) || !isset($params['bsecure'])) {
			return $response;
		}
		...
		// Check and include relevant API classes based on WooCommerce version
		if (version_compare($wc_version, '3.6.0', '>=')) {
			// WooCommerce 3.6.0+
			include_once WC_ABSPATH . 'includes/api/class-wc-rest-system-status-controller.php';
		} else {
			// WooCommerce pre-3.6.0
			WC()->api->includes();
			WC()->api->register_resources(new WC_API_Server('/'));
		}
		...
		// Store variation data in new variable
		if (isset($response->data["type"]) && 'variable' === $response->data["type"]) {
			// write_log('Product is of type variable.');
			if (!empty($response->data["variations"])) {
				// write_log('Variations found: ' . print_r($response->data["variations"], true));
				foreach ($response->data["variations"] as $variation) {
					// Get product variation using wc_get_product
					$product = wc_get_product($variation);
					if ($product) {
						$product_data = $product->get_data();
						$variations_objs[] = $product_data;
					} else {
						// write_log('Failed to get product for variation ID: ' . $variation);
					}
				}
			} else {
				// write_log('No variations found.');
			}
		} else {
			// write_log('Product is not of type variable or type is not set.');
		}
		...
		if (!empty($variations_objs)) {
			...
			foreach ($variations_objs as $key => $variations_obj) {
				foreach ($variationKeys as $variationKey) {
					if (array_key_exists($variationKey, $variations_obj)) {
						// $new_response["variations"][$key][$variationKey] = $variations_obj[$variationKey];
					}
				}
			}
		...
		// write_log('Final Response Data: ' . print_r($response->data, true));
	...
	public function bsecure_action_woocommerce_order_status_changed($order_id, $force = false){	
	...
	public function wc_order_add_discount( $order_id, $title, $amount, $tax_class = '' ) {
	    ...
	    $discounts = ($order->get_items( 'fee' )); 
		if(!empty($discounts)){
			foreach ($discounts as $key => $disc) {
				if($disc->get_name() == $title){
					if ( method_exists( $disc, 'get_id' ) ) {					
						$order->remove_item($disc->get_id());
						$order->calculate_totals();	
					}
				}
			}
	...
	public function bsecure_action_after_post_updated($post_id, $post_after, $post_before){		
		...
		if($post_after->post_status == 'publish' || $post_after->post_status == 'trash'){
		    ...
		    if(is_admin()){	 	    	
		    	$isDelete = ($post_after->post_status == 'trash') ? true : false;
		    	$this->send_info_to_bsecure_via_webhook($product,$isDelete);
		    }
		}
	}
	...
	*/
	...
	public function send_info_to_bsecure_via_webhook($products, $isDelete = false){	

		$product_ids = [];	
		$store_id = get_option('wc_bsecure_store_id', '');
		$base_url = get_option('wc_bsecure_base_url', '');
		$base_url = ($base_url == Bsecure_Checkout::BSECURE_LIVE_BASE_URL) ? Bsecure_Checkout::BSECURE_LIVE_WEBHOOK_URL : $base_url;
		...
		$webhook_endpoint = ($isDelete) ? '/webhook/pim/product-deleted' : '/webhook/pim/product-updated';
	...
	*/
	...
	public function updateOrderStatus($order,$order_data){

		$order_placement_status = !empty($order_data->placement_status) ? $order_data->placement_status : '';
	    $order_status = $this->woocommerceStatus($order_placement_status,$order_data);
	    $order->calculate_totals();
	    $return = $order->update_status($order_status,'',true);	    
	    return $return;
	}


	public function getConfimredStatus($order_data){

		$order_status = Bsecure_Checkout::STATUS_PROCESSING;

		$wc_order_statuses = wc_get_order_statuses();

		if(array_key_exists('wc-'. Bsecure_Checkout::STATUS_CONFIRMED, $wc_order_statuses)){
			if(!empty($order_data)){
				if(!empty($order_data->payment_method)){
					if($order_data->payment_method->id == 1){ // COD = 1
						$order_status = Bsecure_Checkout::STATUS_CONFIRMED;
					}
